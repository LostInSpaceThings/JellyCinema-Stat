...
foreach ($watched_list as $item) {
    $genres = $item['Genres'] ?? [];
    $isAnimated = in_array('Animation', $genres);
    ...
    if (!empty($genres)) {
        foreach($genres_tracked as $genre_name => $count) {
            if (in_array($genre_name, $genres)) $genres_tracked[$genre_name]++;
        }
    }

    // Track Time
    if (isset($item['RunTimeTicks'])) {
        $marathon_minutes += $item['RunTimeTicks'] / 600000000;
    }
...
$milestone_percent = ($total_milestones > 0) ? ($unlocked_count / $total_milestones) * 100 : 0;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .level-label { font-size: 0.9rem; text-transform: uppercase; letter-spacing: 4px; color: var(--milestone-blue); font-weight: bold; text-shadow: 0 0 10px rgba(0, 210, 255, 0.3); }
        ...
        .next-level-text { color: var(--milestone-blue); font-weight: 800; text-transform: uppercase; letter-spacing: 1px; text-shadow: 0 0 10px rgba(0, 210, 255, 0.3); }
    ...
    <div class="recent-list">
        ...
        ?>
            <div class="recent-item">
                ...
                <div class="query-time"><?php echo date('M d, Y', strtotime($item['UserData']['LastPlayedDate'] ?? 'now')); ?><br><?php echo date('h:i A', strtotime($item['UserData']['LastPlayedDate'] ?? 'now')); ?></div>
            </div>
        <?php endforeach; ?>
    </div>
